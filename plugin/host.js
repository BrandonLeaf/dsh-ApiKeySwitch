...
// - 注册 Client RPC：status / list / models / catalog / auto / switch / set / save / add / remove
// - 切换项目时同步默认模型路由（agentDefaultModel.saveSelection）
// - 按会话工作目录自动切换（meta.dirs 目录绑定，一个目录只能绑定一个项目）
//
// DSH 0.1.5 适配要点：
...
return {
  apply(ctx) {
    ...
    // 守卫：有"其他会话"正在执行时返回阻止结果（ok:false + guard:true），否则 null。
    ...
    ctx.effect(() => harness.handle('models', async () => {
      const providers = await liveCatalog()
      return { ok: true, providers: providers, catalog: providers, defaultRoute: liveDefaultRoute() }
    }))
    ctx.effect(() => harness.handle('catalog', async () => {
      const providers = await liveCatalog()
      return { ok: true, providers: providers, catalog: providers, defaultRoute: liveDefaultRoute() }
    }))
    ctx.effect(() => harness.handle('auto', async (args) => {
      const result = await autoSwitchByDir(args || {})
      if (result.ok) return result
      const status = await collectStatus()
      return Object.assign({}, result, status)
    }))
    ctx.effect(() => harness.handle('pickDir', async () => pickDirectory()))
