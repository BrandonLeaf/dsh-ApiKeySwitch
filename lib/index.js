...
const BASE = [
  ...
  { id: 'demo1',          remark: '示例项目 1',     provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL, ref: 'DSH_DEMO1_API_KEY' },
  { id: 'demo2',          remark: '示例项目 2',     provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL, ref: 'DSH_DEMO2_API_KEY' },
  { id: 'demo3',          remark: '示例项目 3',     provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL, ref: 'DSH_DEMO3_API_KEY' },
  { id: 'demo4',          remark: '示例项目 4',     provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL, ref: 'DSH_DEMO4_API_KEY' },
  { id: 'demo5',          remark: '示例项目 5',     provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL, ref: 'DSH_DEMO5_API_KEY' },
]
...
export function apply(ctx) {
  ...
  async function collectStatus() {
    ...
    return { activeProfile, maskedKey: maskKey(active?.value), route, profiles }
  }
    ...
    try {
      ...
      switch (action) {
        ...
        case 'models':
          ...
          break
        case 'switch':
          result = await switchProfile(args.profile)
          break
        case 'set':
      ...
      if (result.ok !== true) {
        const status = await collectStatus()
        result = { ok: false, error: result.error, ...status }
      }
  ...
  if (tools !== undefined) {
    const profileParam = {
      ...
      required: true,
      enum: BASE_IDS,
      description: '项目 id：' + BASE.map((p) => p.id + '(' + p.remark + ')').join('、') + '；设置页新增的自定义项目也可切换',
    }
    ...
    tools.register(defineTool({
      name: 'api_key_switch',
      description: '切换当前生效的 API Key 到指定项目。同时把该项目的模型供应商与默认模型同步为默认路由，下一次模型请求立即生效，无需重启。',
      parameters: { profile: profileParam },
      output: { schema: { type: 'json' }, render: renderJson },
      execute: async (args) => switchProfile(args.profile),
    }))
    tools.register(defineTool({
      name: 'api_key_set',
